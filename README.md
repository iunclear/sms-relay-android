...
优化包位于 `app/build/outputs/apk/release/app-release.apk`。GitHub Actions 使用仓库 Secrets 中的固定发布密钥，对调试包与优化包使用同一证书签名；后续 GitHub Release APK 可以直接覆盖升级。首页的“后台电池设置”会打开系统设置，可将应用设为不受限制的后台运行。推送到 `main` 后，GitHub Actions 会自动上传调试包与优化包，并创建或更新与版本号对应的 GitHub Release。

`v1.1.0` 及更早版本曾使用每次构建都不同的临时 Debug 签名，因此首次迁移到 `v1.1.1` 需要卸载旧 APK 后重新安装；从 `v1.1.1` 起的 Release 之间可直接升级安装。
